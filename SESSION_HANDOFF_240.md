...
Ryan asked how to stop this class of error recurring. The answer is documentary, not attentional:
**`detachments.json` has never been documented.** `40K_Data_Dictionary.md` stops at Session 19
addenda and carries no entry for a detachment record's nine fields, so a census reads only the
fields it already knows about. Separately and worse: 24 enhancements with zero legal bearers was
explained by an inference about designer intent rather than treated as evidence of an unread field.

**B129 opened** — one tooling turn: document the undocumented outputs field by field, flagging free
text that can carry rules; require a field-coverage statement at the head of every census; and add
a `rules_assertions.py` gate failing on any enhancement with no eligible bearer absent a named
exemption. Standing rule: **an impossible result means widen the read, never explain the result.**
S241 retasked from B123 to B129; B123 follows it.
...
| `40K_Decision_Log.md` | `7f3b6acc88be` | D334, D335, D336 appended |
| `DECISION_INDEX.md` | `299749acda86` | D334, D335, D336 one-liners appended |
| `OPEN_ITEMS_BACKLOG.md` | `d50b7b3a61a3` | B125–B129 opened; B93 re-scoped; B123 decided; B116 reclassified; 22 → 27 |
...
22 open at S239 close; **27 open at S240 close** (B125, B126, B127, B128, B129 opened; nothing
closed).
...
Nothing is decision-blocked. B129 is next, then B123.
