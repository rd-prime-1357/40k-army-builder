...
## D225 — Baseline reconciliation S144→S145: B67's "one commit" corrected, three source-file edits verified, `unit_loadouts.json` regenerated (S145)

**Turn type: data-only.** One generated output changed (`unit_loadouts.json`, re-banked); no
parser, engine, or `index.html` change. Substantial verification work preceded the regeneration —
recorded here since it's what makes the regeneration trustworthy, not because it changed anything
itself.

### Baseline-open found real drift, not a bad sync

Hashes for the two files S144 explicitly verified — `Space_Marines_web.txt` and
`Chaos_Space_Marines_web.txt` — no longer matched that handoff. Ryan had edited both further between
sessions (space-saving trims, same treatment as the stratagem-section removal D224 already verified
safe). `Space_Wolves_web.txt` was entirely absent — not a nice-to-have: it is one of `repro_check.py`'s
five ordered web-passes (SM → DG → BT → DA → SW) and a `P4_REQUIRED_SOURCES` entry, so its absence
failed two gates outright (`repro_check`, `rules_assertions`) rather than just tripping a census check.

### B67 — the "single commit" premise was wrong; corrected

D223 recommended `git commit --amend` on the belief the repo's entire history was one commit.
`repo_check.py` was absent from the project mount (as it has been since S141/S142 — pulled fresh from
the repo, established practice); cloning the repo directly for a real history read found **249
commits**, not one — D223's premise was false. Good news underneath the correction: `Unit_Weapons.csv`
and `wh40k_core_rules.md` were each added in exactly one commit and never touched again since, so the
exposure is contained to those two commits specifically, not smeared across the tree. Ryan deleted
both files directly from the repo; confirmed gone from HEAD via the GitHub API. This closes B67's
original ask — nothing serving the live site or a normal repo browse exposes GW text anymore. The two
commits that introduced them are still reachable in history (a real difference from D223's "no earlier
history to preserve" framing) — full removal needs a history rewrite (`git filter-repo` / BFG) and a
force-push, which is a bigger, separate, optional action, not attempted here. Filed as B67b, low
priority, Ryan's call whether it's worth doing. Precedent already exists in this repo either way: an
early batch of `_web.txt` files was committed by mistake and deleted the same way — HEAD-only — years
before this session, and is still sitting in that early history today.

### Three source-file edits verified against the real pipeline

Ryan supplied a complete `Dark_Angels_web.txt` (replacing an incomplete prior version) and a complete
`Space_Wolves_web.txt` (generated by a script he intends to use for regenerating the remaining
`_web.txt` files going forward — LF line endings, unlike the CRLF convention of the hand-sourced
files; initially uploaded as `Space-Wolves_web.txt`, renamed by Ryan to the required underscore form).
Ran the real pipeline — first with Space Wolves still missing (SM/DG/BT/DA only, to unblock partial
verification), then complete once it arrived — and diffed the result against the then-committed
`unit_loadouts.json`. Every difference traced to a specific, understood cause, none left unexplained:
twenty were Space-Wolves-eligible named characters (Logan Grimnar, Ragnar Blackmane, Njal Stormcaller,
and others) that had never carried a `_defaults_source` tag because no working Space Wolves file had
ever run; one was a genuine pre-existing Dark Angels data bug on the Ravenwing Dark Talon
(`000000240`) — the committed defaults were missing the model's second Hurricane bolter — confirmed
directly against `Dark_Angels_web.txt`'s own composition text, which the fix now matches. No parser
errors on any pass, no differences left unaccounted for. `Space_Marines_web.txt` was separately
re-edited by Ryan (via ChatGPT, not yet the new script) since D224's check; re-verified the same way —
clean.

### `unit_loadouts.json` regenerated

124,652 → 125,329 bytes. Manifest reissued. Full baseline: **23/23 gates, 104/104 assertions.**
`Chaos_Space_Marines_web.txt` untouched this session — still awaiting its own scoped data-build turn.

### Capacity — 96% as of this session (Ryan-reported)

After `Chaos_Space_Marines_web.txt` (net new) and the Dark Angels replacement. Recommendation: do not
batch-regenerate the remaining hand-sourced `_web.txt` files (Black Templars, Death Guard, a rerun of
Space Marines) through the new script in one pass — each is load-bearing for a built faction; do them
one at a time, each its own verified data-only turn, the way Dark Angels and Space Wolves were handled
here. The still-unactioned decision-log archive split (flagged since D211/P4 step 1) is very likely a
bigger, lower-risk lever than re-saving already-working files for a line-ending saving alone.

### Also found, not from this session's work

`40K_Data_Pipeline_Process_v0_6.md` differs from its repo copy — the project-area copy carries the
B56a chapter-points scoping steps (`--scope-to-army --append`) the repo copy lacks. Pre-existing sync
gap, not touched here; flagged for Ryan's next repo batch upload.

**Decisions needed (Ryan):** none blocking. Whether/when the B67b history purge is worth doing;
confirm the one-at-a-time approach for Black Templars/Death Guard/Space Marines script regeneration
whenever you're ready to start them.

**Changed:** `unit_loadouts.json`, `pipeline_manifest.json`, `40K_Decision_Log_v3_0.md` (this entry),
`DECISION_INDEX.md`, `OPEN_ITEMS_BACKLOG.md`, `NEXT_SESSION_PROMPT.md`, `SESSION_HANDOFF_145.md`.
`Space_Wolves_web.txt`, `Dark_Angels_web.txt`, `Space_Marines_web.txt` — Ryan's edits, verified,
unchanged by Claude.

**Net new:** none.

**Repo custody:** `unit_loadouts.json` and `pipeline_manifest.json` are repo-eligible pipeline
outputs, changed — belong in Ryan's next batch upload. `Space_Wolves_web.txt` and the updated
`Dark_Angels_web.txt`/`Space_Marines_web.txt` are GW-derived source, excluded under the existing
`.gitignore` pattern, no repo action. `Unit_Weapons.csv` and `wh40k_core_rules.md` confirmed removed
from the repo's HEAD.

---

## D226 — Process rule: pause and ask before a `_web.txt` regeneration turn (S145)

**Turn type: process, no code or data touched.** Ryan's instruction for how S146+ should handle his
planned Black Templars / Death Guard / Space Marines script regeneration (see D225, P4): don't assume
the new file is ready and don't proceed on stated intent. When a session reaches the point of starting
one of these regeneration turns, Claude pauses, explicitly asks Ryan to load that faction's new
`_web.txt` file, and waits for it before running anything. Same shape as the one-at-a-time rule
already set in D225 — this is the concrete mechanic that enforces it, so a session doesn't quietly
start a regeneration turn against a stale file because the file "should" be there.

**Changed:** `40K_Decision_Log_v3_0.md` (this entry), `DECISION_INDEX.md`, `NEXT_SESSION_PROMPT.md`,
...
- **D264** — Manifest reconciliation (S174, tooling-only, session open). `baseline.sh --fetch` failed
  at the first gate: the repo's `pipeline_manifest.json` recorded `02c6632e...` for
  `SESSION_HANDOFF_172.md`, but the actual file committed to the repo hashes to `dbaecf1...`.
  Verified two independent ways (`git clone` and the `codeload` tarball baseline.sh itself uses) —
  both returned the same, real content, ruling out a fetch fluke. Git history dates the handoff
  commit at 06:37 and the manifest commit that added its entry at 07:15 the same session (S173) —
  the manifest entry was written from a copy of `SESSION_HANDOFF_172.md` that was not the one already
  in the repo. Root cause unconfirmed (most likely a stale-mount or duplicate-upload artifact, the
  same failure class the standing constraints already name); the S173-local copy that produced the
  wrong hash no longer exists to inspect. Diff-checked all 128 guarded entries against a fresh repo
  clone — this was the only one affected. Regenerated the manifest against the verified clone (ground
  truth) rather than guessing at the correct hash. Corrected `pipeline_manifest.json` is delivered
  this session for Ryan to push; until it lands, `repo_check` will keep flagging this one entry.

- **D265** — B76 shipped (S174, tooling-only). Dropped the frozen version suffixes from the five
  versioned docs: `40K_Decision_Log_v3_0.md` → `40K_Decision_Log.md`,
  ...
  `40K_Data_Dictionary_v2_0.md` → `40K_Data_Dictionary.md`. Content of each file is unchanged — this
  is a rename, not an edit. Updated the two live scripts that reference these names by name
  (`pipeline_manifest.py`'s `GUARDED` list and `DECISION_LOG` constant; `repo_check.py`'s `DOC_FILES`
  list) and the two live doc cross-references (`DECISION_INDEX.md`'s header note;
  `OPEN_ITEMS_BACKLOG.md`'s P4 entry on a possible future decision-log split). Left every historical
  decision-log entry, the closed-backlog history, `PROCESS_IMPROVEMENT_PLAN.md`'s S126-era file list,
  and every session handoff untouched — those describe the past under the names that were true then,
  per B76's own scoping note. The five renamed files and the five updated live files are delivered
...
- **D268** — E26 re-scoped from source; no CSM data change exists (S177, analysis-only — no code, no data, no `index.html` change). Opened as the E26 engine turn; the scoping pass materially corrected E26 and found the CSM "data gap" the S176 handoff implied does not exist. **Findings, source-derived this session (baseline green 29/29 with GW sources loaded).** (1) The live attach engine (`permitsCoLeader`, index.html 4271) returns false for a bare Support — empty `coLeaderWith`, `coLeaderAny` false — against every Leader, so Master of Executions and the whole SM Support family can attach as a *sole* leader but cannot co-attach today. That is the real defect, and it is **engine**, not data. (2) Enforcing 19.01 is a layered model: the base rule (one Leader slot + one Support slot, two total, already the D157/D158 cap) is the ceiling; the datasheets decide which specific pairings are legal *within* it, in three shapes — a **named list** on a Support (`co_leader_eligible_with`: Lieutenant → Captain/Chapter-Master-rank, Cato → Calgar only), a **generic flag** (`co_leader_any`: the six DG Plague characters, meaning "second Leader"), and a **leader-rule cross-reference** where a Leader's own eligible list names an attach-capable character (Huron Blackheart → Masters of the Maelstrom — the *only* such cross-reference across all 16 built factions, verified). (3) Two Supports on one bodyguard is illegal by the base rule (Ryan's call) and the engine must actively refuse it, because the legacy 10th-ed `co_leader_eligible_with` lists cross-reference each other (Apothecary's list names Lieutenant) and `permitsCoLeader` would otherwise wave it through. **Three prior-session/this-session assumptions corrected against source.** (a) My proposal to discard the `co_leader_eligible_with` name lists as "dead" was wrong — they are the datasheet combination restriction (Ryan: base rule sets the ceiling, unit rules govern combinations; see the SM Lieutenant, which pairs only with Captain-rank). (b) My proposal to add Masters of the Maelstrom to Huron's `co_leader_eligible_with` was wrong — the MFM already carries it in Huron's leader rule (`leader_eligible_units`), so it needs no data change, only an engine read. (c) My proposal to set `co_leader_any = true` on Master of Executions was wrong twice over: B73 deliberately cleared MoE's footer (Support units don't carry the stale Leader clause), and **being Support already grants** the any-Leader pairing the flag was meant to grant; setting the flag would also be the *Leader* reading of MoE, which **D267's MFM-wins rule discards**. **MoE type is not an open question — D192 + D267 already govern it.** The Wahapedia datasheet heads MoE's ability `LEADER` with the "even if one other CHARACTER" clause; the MFM types it `SUPPORT`; MoE is one of the 14 Leader→Support flips B73 shipped. Per "MFM is authoritative wherever both exist," **MoE is SUPPORT, no new decision needed.** Full CSM source pass: only two CSM datasheets carry a co-attach footer (Master of Executions; Exalted Champion, which is *not built* — no MFM points), both the generic CHARACTER shape; MoE is correctly typed Support with a correctly-cleared footer, so **the CSM attach data is already right and there is nothing to regenerate.** The deferred D144 CSM `co_leader_any` population is thereby resolved-as-unnecessary: verified from source that no built CSM unit needs it. **E26, re-scoped — engine-only, no data dependency.** Requirements: (i) a Support-typed unit fills the Support slot and pairs with any single Leader by the base rule (fixes the bare-Support false); (ii) keep the DG `co_leader_any` second-Leader path; (iii) read a Leader's `leader_eligible_units` naming another attach-capable character as a co-attach permit (Huron → MotM); (iv) add a same-type cap so two Supports (or two Leaders without a lift) cannot stack on a stale cross-listed name; all four expressed as `rules_assertions.py` checks. Nothing banked but these documents. Open count unchanged 13 → 13 (E26 re-scoped in place, not resolved).

- **D269** — E26 shipped (S178, engine-only). `permitsCoLeader` rewritten to enforce the D268 one-leader-one-support stacking model. Four requirements implemented: (R1) a bare CHARACTER Support (empty `coLeaderWith`, no `coLeaderAny`) pairs with any single Leader by the base rule — fixes the core defect where `permitsCoLeader` returned false for Master of Executions and the whole SM Support family against every Leader; (R2) the DG `co_leader_any` second-Leader path kept — two Leaders allowed only if at least one has the flag (the six DG Plague characters), never same datasheet; (R3) a Leader's `leader_eligible_units` naming a non-CHARACTER Support reads as a co-attach grant — Huron Blackheart → Masters of the Maelstrom, the sole such cross-reference across all 16 built factions; MotM has no self-grant (empty `coLeaderWith`, not CHARACTER) so only Huron can share a bodyguard with it; (R4) same-type cap: two Supports always illegal, two Leaders without a `co_leader_any` lift also illegal, actively overriding the stale cross-listing where Apothecary's `co_leader_eligible_with` names Lieutenant. The `isCharacter` flag (from the CHARACTER keyword) distinguishes R1 (bare CHARACTER Support → any Leader) from R3 (bare non-CHARACTER Support → only via Leader cross-reference). Named `co_leader_eligible_with` lists preserved as the datasheet combination restriction (Lieutenant → Captain-rank, Cato → Calgar only). `leaderAbilityName` added to the `allUnits` view object, read from `mg.leader_ability_name`. Assertion E26 added: 9 structural shape checks on the source code and 10 legality cases tested with symmetry (Captain+Lieutenant legal, Lieutenant+Librarian illegal, Lieutenant+Apothecary illegal, Cato+Captain illegal, Cato+Calgar legal, MoE+Captain legal, Huron+MotM legal, MotM+Captain illegal, two DG Plague characters legal, same datasheet always illegal). `index.html` v6.12 → v6.13; 75/75 tier-A assertions pass (112 total). Open 13 → 12 (E26 shipped).

- **D270** — E27 shipped (S179, UI-only). `renderDetail`'s attach-panel section heading and hint, and `leaderSectionHtml`'s datasheet-modal section heading, both rewritten to read `leaderAbilityName`/`leader_ability_name` (Leader or Support) instead of a hardcoded "Leader" string. All model groups on a given unit confirmed to share one ability name (no unit mixes Leader/Support across model groups, checked against all 131 model groups carrying a non-null value); heading logic uses a defensive fallback to "Leader" for the hypothetical mixed case. Two of the ticket's three named candidate sites investigated and found to need no change: the list-panel attached-unit row (`${leader.unit_name} (${entry.unit_name})`) carries no role word, and the JSON save/export schema (`serializeEntries`/`buildRecord`) carries no role field — `attached_to` is a listId reference only. **Finding that reshaped the ticket's risk profile:** the Rules-section dedup filter in both modal builders checks the literal string `'Leader'` against each model group's `rule_names`, and had to be left untouched rather than switched to `leader_ability_name` — the datasheet's own printed ability box is always literally "Leader" (129 of 131 model groups; the remaining 2, both Masters of the Maelstrom, carry a bespoke named ability with neither word), and the literal string "Support" appears zero times in any built `rule_names` array. `leader_ability_name`'s Leader/Support split is sourced from the Munitorum Field Manual's own LEADER/SUPPORT block headers (`mfm_points_parser.py`) — a real, official GW distinction, but from a different document than the datasheet card, confirmed via web search that 40k's core rules print only one ability named "Leader" with no separate "Support" ability text anywhere. Switching the Rules-section filter to the classification field would have double-printed the ability for every Support-classified unit except MotM. Assertion E27 added: structural-shape check only (no legality logic changed) — confirms both headings read the ability-name field and that the old hardcoded strings are gone, not merely shadowed. `index.html` v6.13 → v6.14; 76/76 tier-A assertions pass (113 total). Open 12 → 11 (E27 shipped).
- **D271** — GUARDED-list close-out fix; not a ticket (S180, tooling-only — no engine, no data, no `index.html` change). At S179 close, `pipeline_manifest.py`'s `GUARDED` list was found to end at `SESSION_HANDOFF_176.md`, three sessions behind — `177`, `178`, `179` had never been appended, so the plain `pipeline_manifest.py` gate (run every session by `baseline.sh`) had nothing to check them against and stayed green while all three sat completely unguarded; only `--freshness-check` (which follows the latest-handoff pattern regardless of `GUARDED`) had any visibility into the gap, and only for the single newest file. **Fix went beyond re-adding the three names.** A plain re-add closes today's instance but not the failure class: a coverage check that scans the *local* directory for unguarded `SESSION_HANDOFF_N.md` files would miss a repeat, because Ryan's routine housekeeping deletes old handoffs from the project area — exactly the files most likely to have been silently dropped from `GUARDED` are also the ones no longer resident locally to be scanned. Verified this concretely: simulating the pre-fix `GUARDED` list against the real project-area copy found only `179` (still resident); the same simulation against the full fetched repo tarball (`codeload.github.com/rd-prime-1357/40k-army-builder`) found all three (`177`, `178`, `179`), since the repo retains full history regardless of area cleanup. **What shipped:** `pipeline_manifest.py` — `SESSION_HANDOFF_177.md`/`178.md`/`179.md` added to `GUARDED` (134 guarded files total); new `unguarded_handoffs(d)` helper flags any `SESSION_HANDOFF_N.md` present in `d` that `GUARDED` doesn't list; folded into both `check()` (local-directory runs) and `check_overlay()` (checked against `fetched_dir`, the full unpacked repo tree — the version that actually matters, since it is the only point in the pipeline guaranteed to see the complete handoff history regardless of what routine cleanup has removed locally). Confirmed the `check_overlay` version would have failed loudly on S178's very first baseline had it existed then, using the real fetched repo tree with a simulated pre-fix `GUARDED`. **Rejected alternative:** replacing the static per-filename list with the same `latest_handoff()` discovery pattern `--freshness-check` already uses, as the next-session prompt raised as a live option. Declined because `build()`'s current design deliberately raises on a *missing* guarded file — the mechanism that would catch a handoff lost from the repo — and full auto-discovery ("guard whatever's present") can only ever find files, never notice one has gone missing; that trades a real detection capability for the one this session fixes. The static list stays; forgetting to update it is now loud instead of silent. `python3 pipeline_manifest.py --write` then `--freshness-check` run clean (134 files, decision log and `SESSION_HANDOFF_180.md` both match). **Also backfilled this session, not scoped to the ticket-less fix above:** `BACKLOG_ARCHIVE.md` was missing full-history entries for B73 (S176) and E26 (S178) — the archive jumped from B76 (S174) straight to E27 (S179). Backfilled both from D260/D266/D267 (B73) and D268/D269 (E26), inserted in chronological order ahead of the existing E27 entry; content only, no re-ordering of anything already present. No ticket opened or closed by this session — this was close-out debt, not backlog work; open count unchanged at 11.

- **D272** — E23 scoped from source (S181, analysis-only — no engine, no data, no `index.html` change). Read D209's original E23 filing directly (it sits inside the D209 entry, not under its own heading) rather than trusting the S181 prompt's paraphrase, and re-derived `detachment_effects.json`'s current schema from the live file rather than the prompt's count. **Correction: the prompt's "sixth effect kind" is wrong.** The schema's `_meta.effect_kinds` block and every one of the 9 built entries confirm exactly four kinds in use today — `battleline`, `forbid`, `unlock`, `warlord` — matching D204's four-kind schema with `require` dropped. D209 called Headhunter Task Force a *fifth* kind candidate at S134, and nothing shipped between then and now added a fifth. If built as a schema addition, Tank Ace's grant is the fifth kind, not the sixth. **The load-bearing finding: E4 and E9 test Character status through two different, unit-definition-scoped representations, and neither has a per-list-entry override hook — which Tank Ace's grant structurally requires.** E9 (`isCharacter`, `eligibleWarlordEntries()`) is computed once per unique `unit_name` in the faction's unit index (off `keyword_names`) and looked up by name for every list entry sharing that name — one Land Raider's flag is every Land Raider's flag. E4 (`enhancementTypeEligible`, three call sites) reads `entry.unit_type` — the raw `units.json` field — copied onto the list entry at every construction site (`index.html` ~1900/1965/2318/3989/4116/4157), never through `effectiveUnitType()`, which exists only for D204's blanket battleline grant and is not consulted by any enhancement code path. Tank Ace is neither: the grant applies to **up to three player-picked instances**, not every instance of a name and not a detachment-wide blanket, so a data-only fix (append "Character" to a unit's keyword list, or extend `effectiveUnitType()`'s blanket pattern) would either elevate every copy of the vehicle army-wide or require no player choice at all — both wrong. Confirmed no data conflict this creates: none of the 28 `Vehicle`-type units in the generic Adeptus Astartes block (Land Raider, Land Raider Crusader, Stormhawk Interceptor, Firestrike Servo-turrets, Drop Pod, Ballistus Dreadnought, Storm Speeder Hammerstrike, Dreadnought, Storm Speeder Hailstrike, Razorback, Brutalis Dreadnought, Invictor Tactical Warsuit, Vindicator, Stormtalon Gunship, Stormraven Gunship, Storm Speeder Thunderstrike, Gladiator Reaper, Gladiator Valiant, Land Raider Redeemer, Impulsor, Gladiator Lancer, Predator Annihilator, Predator Destructor, Redemptor Dreadnought, Repulsor, Repulsor Executioner, Rhino, Whirlwind) is already `unit_type: Character`, so no overlap case exists to reconcile. **Mechanism decided (dev-manager call, reversible): hybrid, not a pure fifth effect-kind row.** (1) A new declarative `detachment_effects.json` kind (name TBD at build time, e.g. `character_grant`) carries the detachment-scoped static facts — which unit_types/exceptions count as Tank Ace-eligible, and the count cap (3) — fitting the existing schema shape the same way `unlock`'s numeric `points_cap` does. (2) The player's actual picks are new, purely-additive per-list state — an array of `listId`s, length-capped by the detachment's grant — added to `list_store.js` the same way `warlord_entry_id` (v1) and `force_disposition` (v3) were: absence reads as "none elevated," which is exactly what an older record already meant, so **no schema version bump**. **Storage/reset behaviour decided by existing precedent, not asked to Ryan:** re-validated continuously, the same shape as `recomputeWarlord()` — any picked `listId` that stops being eligible (leaves the list, its detachment is deselected, the cap is exceeded) is silently dropped on every recompute, no confirmation prompt. Checked for a "Muster" phase to gate against: the app has none — `index.html` mentions "Muster" only in rules-citation comments, never as a modeled step — so picks stay editable continuously, identical to Warlord and Enhancement selection today. This resolves both open sub-questions the S181 prompt flagged as possibly Ryan-facing; neither turned out ambiguous once checked against what two already-shipped mechanisms already do. **Engine touch points identified for the build turn:** `eligibleWarlordEntries()` needs an OR against the new per-entry pick array alongside the existing `x.unit.isCharacter` test; `canAssignEnhancement`/`enhancementTypeEligible`'s three call sites (~3194, ~3375, ~3689) need an effective per-entry type (raw `unit_type`, or `'Character'` when this entry's `listId` is a live pick) in place of the raw field — mirroring `effectiveUnitType()`'s "compute an overlay, never touch the raw record" shape, but at per-entry rather than per-detachment granularity, since no existing function does that today. **Still blocked on a data turn, not run this session (turn-typed analysis-only; sources not loaded — `baseline.sh --fetch` alone leaves `repro_check`/`units_repro_check`/`detachments_repro_check` tier-B-skipped):** exact keyword-grant wording must be confirmed per D209's own standard (source-first, not carried forward) across all six armies (Space Marines, Black Templars, Blood Angels, Dark Angels, Deathwatch, Space Wolves) — "most" Adeptus Astartes Vehicles implies a carve-out whose exact membership and whether it is worded identically in all six is unconfirmed, and the "up to three" cap itself should be reconfirmed per army rather than assumed uniform. No ticket shipped this session; E23 re-scoped in place, its backlog text rewritten with this plan. Open count unchanged at 11.

- **D273** — E23 data-turn: `HEADHUNTER TASK FORCE` source facts confirmed across all six armies (S182, data-only — no engine, no data-file change, no `index.html` change; `index.html` stays v6.14). Baseline `--fetch --data-turn` green **29/29** with GW sources loaded at open and re-run clean at close. This turn's job was the record a build turn will consume, not the schema row. **All four facts confirmed, but D209's and D272's "most Adeptus Astartes Vehicles" is imprecise and is corrected here.** **(1) Grant wording — identical across all six, because it is one Space Marines detachment, not six copies that could diverge.** The Tank Ace grant text is present verbatim in exactly one authoritative source, the Space Marines Faction Pack v1.0 (`## p7 — HEADHUNTER TASK FORCE`), and the Wahapedia `Detachment_abilities.csv` carries it once under `faction_id=SM` (row 246, rule "Target Sighted"); the two agree word-for-word. The five successor armies do not reproduce it — the Dark Angels Faction Pack does not contain it, the Space Wolves web file's only "headhunter" hit is an unrelated Wolf Guard Headtakers unit ability, and every army's MFM carries only the detachment name plus its enhancement points (`Astartes Tank Ace 40 pts`), never the grant. In the built data, `detachments.json`'s `rule_text` is **byte-identical across all six records** (SHA-256 `cadd53c18131`, 922 chars) — Space Marines, Black Templars, Blood Angels, Dark Angels, Deathwatch, Space Wolves. So "does wording vary per army" resolves to no, and for a structural reason: there is one rule shared by six armies' detachment pools, not six independently-worded copies. **(2) The Tank Ace-eligible definition is a precise keyword predicate, not "most Vehicles."** Exact rule: Adeptus Astartes Vehicle units *"excluding Fortifications, Drop Pods, Walkers and units that can Fly"* get Tank Ace. This is fully computable from the built data: `Fly` and `Walker` are unit keywords present in `units.json` model-group `keyword_names` (confirmed on the actual flyers and walkers), `Fortification` is a `unit_type`, and Drop Pod carries a `Drop Pod` keyword and unit_name. **The carve-out is therefore an exclusion predicate, and must not be authored as an enumerated unit-name list** — because the eligible set is resolved against each army's own roster and is not the same set in all six. **(3) Count cap "up to three" — confirmed identical in all six** (same source text, same byte-identical `rule_text`). **(4) Detachment keys — all six confirmed** as `<Army>|HEADHUNTER TASK FORCE` (Space Marines / Black Templars / Blood Angels / Dark Angels / Deathwatch / Space Wolves), each `dp:2`, `force_disposition: "PRIORITY ASSETS"`, `enhancements` carrying Astartes Tank Ace 40 / Firestorm Coordinators 20 / Gunnery Honours 20 / Redoubtable Machine Spirit 25. **Per-army eligible pool, resolved from source this session** (pool = each army's block ∪ the generic Adeptus Astartes block for a subfaction; Space Marines maps to the Adeptus Astartes block alone, `is_subfaction:false`). Generic Adeptus Astartes block: **28 `unit_type: Vehicle` units → 16 eligible, 12 carved out** (5 Walkers: Ballistus Dreadnought, Brutalis Dreadnought, Dreadnought, Invictor Tactical Warsuit, Redemptor Dreadnought; 6 that Fly: Storm Speeder Hailstrike, Storm Speeder Hammerstrike, Storm Speeder Thunderstrike, Stormhawk Interceptor, Stormraven Gunship, Stormtalon Gunship; plus Drop Pod). The 16 generic eligibles: Firestrike Servo-turrets, Gladiator Lancer, Gladiator Reaper, Gladiator Valiant, Impulsor, Land Raider, Land Raider Crusader, Land Raider Redeemer, Predator Annihilator, Predator Destructor, Razorback, Repulsor, Repulsor Executioner, Rhino, Vindicator, Whirlwind. **Resolved eligible counts differ per army — Blood Angels is the one that is not 16.** Space Marines 16, Black Templars 16 (no army-specific vehicles beyond the generic block), **Blood Angels 17** (adds Baal Predator; Death Company Dreadnought is excluded as a Walker), Dark Angels 16 (its four own vehicles — Land Speeder Vengeance, Nephilim Jetfighter, Ravenwing Dark Talon, Ravenwing Darkshroud — all Fly and are excluded), Deathwatch 16 (Corvus Blackstar excluded, Fly), Space Wolves 16 (its four own vehicles — Bjorn The Fell-handed, Murderfang, Venerable Dreadnought, Wulfen Dreadnought — all Walkers and excluded). **No eligible unit in any of the six is already `unit_type: Character`/`Epic Hero`** — extends D272's Adeptus-Astartes-only check to every resolved pool, so the grant is never a redundant no-op. **Three build-turn design notes banked, all dev-manager calls, not Ryan-facing:** (a) **Base the eligibility test on the Vehicle *keyword*, not `unit_type: Vehicle`.** Hammerfall Bunker carries the `Vehicle` keyword but `unit_type: Fortification`; a keyword base makes the rule's "excluding Fortifications" clause do real, literal work (Hammerfall is the only Adeptus Astartes case it catches), whereas a `unit_type: Vehicle` base would silently pre-exclude Hammerfall and leave the Fortification clause as dead code — the keyword base matches the rule's own wording and is future-proof if a Fortification+Vehicle unit is ever added. (b) **Encode the carve-out as a predicate on the detachment effect row (exclude keywords `Fly`/`Walker`/`Drop Pod` + `unit_type` `Fortification`), evaluated per list entry on that entry's own keywords — not as a per-detachment name list.** The per-army lists above are ground truth for the build's assertion, not data the engine enumerates. (c) **The "Adeptus Astartes" faction qualifier is currently satisfied by pool construction, not by a keyword check** — built `keyword_names` drop faction keywords, so the predicate cannot test "Adeptus Astartes" directly, but every unit these six armies can field is Adeptus Astartes, so it is safe today; the build turn should add an assertion that no non-Adeptus-Astartes vehicle can ever enter these pools and silently become Tank Ace-eligible. No ticket shipped; E23's data dependency is now cleared and its backlog text updated with these confirmed facts. Open count unchanged at 11.

- **D274** — MFM v1.1 intake: sources banked, layout break confirmed, refresh arc opened (S183,
  doc-only — no engine, no data-file, no tooling change; `index.html` stays v6.14; baseline
  `--fetch --data-turn` green 29/29 at open). GW published a points update; MFM pages now carry a
  per-faction GW version number (v1.1). **Custody: 15 v1.1 captures banked in the private sources
  repo** — pages covering all 16 built army blocks (the SM page carries the six codex-less chapters;
  Victrix Honour Guard / Suboden Khan / Uriel Ventris / Captain Titus spot-checked present) plus
  Emperor's Children (next build) and four beyond policy (Chaos Knights, Drukhari, Grey Knights,
  World Eaters — harmless, retained). All 15 non-empty, uniform new layout; the SM repo copy is
  text-identical to the sample reviewed in-session. **The v1.1 layout breaks the current parser:**
  `parse_mfm` on SM v1.1 finds 154 unit names (25 missed — Rhino, Razorback, Drop Pod among them, so
  name detection, not removals) and parses **zero costs**, vs 179/179 costed on v1_0. Layout deltas:
  bullet prefix on cost lines gone; "▼" markers trail unit names and model counts; inline change
  annotations ("(-10)", "▲ (+10)", "UPDATED" / "FORCE DISPOSITION(S) CHANGED" lines) — final value
  only, per the MFM's own instructions; a blank line now separates LEADER/SUPPORT from the attach
  list; new section headers (UNITS / DETACHMENTS / ENHANCEMENTS / WARGEAR OPTIONS). **The new capture
  is richer than v1_0:** each faction page now carries its DETACHMENTS block (DP, force disposition,
  enhancement costs) inline — the v1_0 SM capture had none. **Standing intake policy, set with Ryan:**
  (a) filenames carry the GW version number, underscore separator per the existing convention — the
  uploads use "v1.1" (dot) and one has a space ("MFM_Chaos Daemons"); D275 confirms filenames accepted as-uploaded (dots, spaces) — no rename step; (b) **keep every version** — a
  superseded MFM file is never deleted; reconciliation needs old and new side by side, and decisions
  and assertions that cite an MFM version must stay checkable against source, not a banked diff;
  (c) capture policy per update: built factions whose version bumped, plus the next faction in the
  build queue; unbuilt factions get a fresh capture at build time regardless of version — no
  speculative full-catalogue captures; (d) `source_manifest.json` records which MFM version each
  faction is built from — per-faction version skew (GW partial updates) is expected and explicit, and
  the manifest, not the filename, is what the pipeline reads; (e) **one parser, per-file format
  sniff** (the layout is self-identifying — section headers, the "▼" marker), per-layout readers
  behind it, everything downstream shared; a layout reader is retired once no parsed file uses it,
  with git history keeping it; (f) **sequencing: the MFM refresh arc runs ahead of the E23 build** —
  stale points undercut D0's promise, and adoption can change vehicle rosters, which would stale
  D273's per-army pool ground truth; the E23 build turn re-verifies D273's counts against
  post-adoption data at open. **Three tickets opened:** B87 (tooling — parser v1.1 layout support),
  B88 (reconciliation — `mfm_reconcile.py` generalized across built factions, per-faction delta
  reports covering points, roster membership, attach lists, and the detachment block), B89 (adoption
  — per-faction data-only turns behind a key-level diff gate). Open count 11 → 14.

- **D275** — MFM filename convention: dot separator, spaces allowed (S183, Ryan call). Ryan's uploads
  use dots (`v1.1`) and natural spacing (`Chaos Daemons`), matching his local naming. The underscore
  convention on the v1_0 files was an earlier session choice, not Ryan's preference, and does not bind
  future captures. **Standing convention going forward:** `MFM_<Faction>_v1.1.txt` with dots for the
...
## D283 — B87 shipped: MFM v1.1 layout support in the points parser; a shipped Rubric Marines overcharge fixed in-flight; B90's Legends/Forge-World roster question answered; v1.1 detachment parsing rescoped to B88 (S190)

**Turn type: tooling**, plus one coupled two-value data correction that the tooling fix forced (see below). No engine or assertion change; `index.html` untouched, still v6.15.

**B87 — v1.1 layout support, closed.** `mfm_points_parser.py` now reads GW's MFM v1.1 page layout. Rather than fork the parser, a per-file sniff detects v1.1 (keyed on the ▲/▼ change markers, which appear in no v1_0 file) and a normalization pass rewrites each v1.1-exclusive quirk into the exact v1_0 line shape the existing readers already parse: it drops the leading `UNITS` section header, the standalone `▲`/`▼`/`▲▼`/`▼▲` change markers, and the editorial `UPDATED` / `REQUISITION THRESHOLDS REMOVED` / `FORCE DISPOSITION(S) CHANGED` notes; and it strips the inline `▼ (-10)` / `▲ (+10)` annotations from inside a cost line, leaving only the final printed value (per MFM_Instructions: the ± delta is disregarded for list building). The three cost line-shape readers (COST_RE, COMPOSITION_RE, ADDON_RE) were made bullet-optional so one reader serves both editions; a v1_0 file is never touched by normalization (sniff returns False), so its output is provably unchanged. Result: all 15 banked v1.1 files cost fully — SM 179/179 (0 before), 15/15 files at 100% coverage. 15 v1.1 filenames registered in `source_manifest.json` (accepted as-uploaded per D275 — note the space, not underscore, in `MFM_Chaos Daemons_v1.1.txt`) and added to `FACTION_BY_MFM`. Net-new `b87_check.js` pins the three acceptance facts as executable checks: v1.1 full costing, v1_0 output stable, and the Rubric Marines correction.

**A shipped points bug found and fixed in-flight.** The MFM tier shape `YOUR 1ST TO 3RD UNITS COST` / `YOUR 4TH + UNIT COSTS` breaks the copy price at the 4th copy. The parser had no reader for it — the two tier lines matched nothing, the unit fell through to single-mode, and the last line seen (the pricier 4th+ bracket) became the unit's only price. This shipped live: Rubric Marines (CSM datasheet 000003583, TS datasheet 000001020) was priced at 110/200 (the 4th+ value) for 5/10 models when the correct 1st-to-3rd price is 100/190 — every player's first three units of Rubric Marines overcharged by 10 points. B87 added an `esc4` reader for the shape that emits the 1st-to-3rd price across all three schema copy-tiers (correct for copies 1–3, which is what the current 3-tier schema can hold) and attaches the un-representable 4th+ tier as `_esc4_fourth_plus` for B94, rather than dropping it or letting it overwrite copies 1–3. Because the corrected parser then produced different output than committed `units.json`, `units_repro_check` went red on exactly those two ids — a correct tooling change coupled to stale data. Rather than shim the gate to reproduce a known-wrong value (itself a workaround the session protocol forbids), the two Rubric Marines values were regenerated through the real pipeline and banked into `units.json`; verified that the ONLY change versus committed is those two units' points, both points-only, all other fields byte-identical. `units_repro_check` green. This is the one place this turn touched data; it is the same fix as the parser change, not separable from it. Also mis-parsed by the same shape but not shipped: Grey Knights' Brotherhood Terminator Squad (GK is not a built army).

**B94 opened for the deferred schema decision.** The 1st-to-3rd/4th+ shape is used by 34 units in v1.1 (dedicated transports — Rhino, Razorback, Drop Pod, Impulsor, Chaos Rhino, Raider, Venom — plus Rubric Marines), rare in v1_0 (two units). The 3-copy-tier points schema (`first_unit`/`second_unit`/`third_plus`) cannot represent a 4th-copy break. B94 carries the product/schema call — add a real 4th copy-tier to the schema, the engine lookup, and the Python mirror, or document a fold rule — plus its engine+data+assertion turns. Recommendation recorded: add the real tier; a wrong points value is D0-class and 34 units is not an edge case. Sequenced with B89's adoption so the affected units migrate once.

**B90's last sub-question answered.** Legends/Forge-World datasheets that a chapter's own current MFM prices ARE legal matched-play roster members for that chapter, sourced from the current MFM (never locked to one edition). Verified against source: Astraeus (525/575) and Thunderhawk Gunship (840) are priced identically to ordinary units in all five Tier-2 chapter MFM files, with no Legends marker or separate section. Consequence for B90 turn 2: both units are currently excluded app-wide by `wahapedia_transform.py`'s `source_is_excluded`, because their Wahapedia source is tagged "…(Forge World)" and the exclusion bundles Forge-World with genuine Legends content — two distinct source categories in GW's own data that the code cannot presently tell apart. Turn 2 must carve these out of that exclusion for the five chapters. B90 turn 2 is now fully unblocked on decisions; it still runs after B88/B89 per D274's sequencing.

**v1.1 detachment parsing rescoped from B87 to B88.** B87 as written carried a clause to "parse the v1.1 DETACHMENTS block for B88." Detachment blocks are parsed by a separate parser (`detachment_parser.py`), not `mfm_points_parser.py`; adding a second detachment reader to the points parser would duplicate it and violate single-source. `detachment_parser.py`'s two MFM readers (`MFM_DP_RE`, `MFM_ENH_RE`) assume the v1_0 layout and cannot read v1.1 (DP on its own line, no enhancement bullet). B88 now owns extending that parser with v1.1-layout support, mirroring B87's sniff-and-normalize approach, ahead of producing reconciliation reports. Dev-manager scope call, noted rather than surfaced.

**Rationale:** the parser fix and the two-value data correction are one fix — the parser was wrong and the data it produced was wrong; correcting one without the other leaves the baseline red for no benefit and keeps a live overcharge on the board. The 4th+ tier is captured rather than dropped so B94 has correct input. Open count unchanged at 17 (B87 closed, B94 opened).

## D284 — B88 shipped: MFM v1.1 DETACHMENTS-layout support in detachment_parser.py; mfm_reconcile.py generalized into a per-faction delta tool; B95 opened (S191)

**Turn type: tooling/analysis.** No engine or assertion-logic change beyond registering the new
harness and updating the P4 source census; `index.html` untouched, still v6.15. No data file
regenerated — `ARMY_TO_MFM` still points every built army at its v1_0 file, so `detachments.json` is
unchanged this turn; the new parsing capability is dormant until B89 adopts it, same shape as B87.

**B88 part 1 — v1.1 DETACHMENTS-layout support, closed.** `detachment_parser.py` now reads GW's MFM
v1.1 DETACHMENTS block, mirroring B87's sniff-and-normalize pattern exactly: the same ▲/▼-marker
file-level sniff (a v1_0 file carries none), and a normalization pass over the DETACHMENTS...LEGENDS/
EOF slice only (never the UNITS section, which `mfm_points_parser.py` already owns) that rewrites
v1.1's two-line shape — `NAME` / `<n>DP`, enhancement `Name` / `<n> pts` with no bullet — back into
the exact v1_0 jammed line (`NAME<n>DP`, `• Name<n> pts`) the existing `MFM_DP_RE`/`MFM_ENH_RE`
readers already parse, so the reader itself is untouched. Verified against source before writing any
normalization rule, not assumed from the B87 precedent: every DP line and every enhancement-cost line
in all 15 v1.1 files' DETACHMENTS blocks is immediately preceded by its name line with nothing
interposed, and no blank lines occur inside any block, so a straight adjacent-pair join is correct —
no lookahead-past-noise logic is required. Two quirks found that B87's points-file precedent didn't
need to handle: a bare trailing change marker with no parenthesised delta on a DP line itself
(Thousand Sons' HEXWARP THRALLBAND, `3DP ▲`, DP value 2→3) rather than the inline `(±N)`-annotated
form B87 handles inside a cost line; and a third editorial note string, `UNIQUE TAG REMOVED`
(World Eaters), missed on a first pass keyed only off the Space Marines file and caught once World
Eaters was actually parsed — then confirmed complete by an exhaustive sweep for any other all-caps
short line across all 15 files' DETACHMENTS blocks. Result: all 15 v1.1 files now parse their
DETACHMENTS block cleanly (0 before); every v1_0 file's parse output proven byte-identical to the
pre-edit parser for all 10 files currently in `ARMY_TO_MFM` (structural equality check, not just a
count match). Net-new `b88_check.js` pins v1.1 parsing across all 15 files, v1_0 stability on the two
live-army files, and the two quirks above as executable facts. `rules_assertions.py`'s P4 source
census updated for the new `MFM_v1_1_Reconciliation.md` filename now referenced by `mfm_reconcile.py`.

**B88 part 2 — mfm_reconcile.py generalized, closed.** The old script was a one-off SM-only pass
comparing a since-superseded `mfm_sm.txt` against `MFM_Space_Marines_v1_0.txt`, with several
dependencies on files no longer present (`Source.csv`, `Datasheets.csv`, `out/Unit_Stats.csv`).
Rewritten to compare, for each of the 10 distinct MFM files backing the app's built armies (the
`ARMY_TO_MFM` deduplicated set — the six generic-SM chapters share one file), the v1_0 file the app
was actually built from against the v1.1 file (the newest capture), using `mfm_points_parser.parse_mfm`
and `detachment_parser.parse_mfm_detachments` as the single sources of truth (no parser logic
duplicated). Scope is these 10, not all 15 v1.1 files banked in `source_manifest.json` — the other 5
(Chaos Knights, Drukhari, Emperor's Children, Grey Knights, World Eaters) have no built version to
diff against. Six delta categories per faction: roster (units added/removed), points (tier/bracket
cost changes), wargear (per-item cost changes, plus items added/removed), attach lists (LEADER/SUPPORT
list changes, including outright Leader/Support flips), and detachments (DP, force disposition, unique
tag, and enhancement changes). Every delta classified adopt-mechanically (a numeric value changed on a
record both files agree exists under the same name) vs investigate-first (anything structural — an
added/removed unit, detachment, enhancement, or wargear item; an attach-list change; a force-disposition
or unique-tag change on an otherwise-matched detachment). **A real bug in the first draft, caught before
shipping:** detachment force-disposition and unique-tag changes were initially counted alongside DP
changes as adopt-mechanically, since both sit on the same "fields changed on a matched record" code
path. Force disposition and unique tag are rules-shape properties, not values — a disposition swap
can change which missions a detachment is even legal for — so they were split out into
investigate-first before the report was banked; the fix moved 44 deltas across the 10 factions from
adopt to investigate (initial run: 233 adopt / 27 investigate; corrected: 189 adopt / 71 investigate).
The corrected report also independently reproduced the exact HEXWARP THRALLBAND DP change found while
building the parser (2DP→3DP, classified adopt-mechanically) and several units matching B94's own
`esc4`-shape description (DROP POD, IMPULSOR, RAZORBACK, RHINO, CHAOS RHINO all show a `single→esc4`
mode change in the Space Marines/Thousand Sons sections) — cross-validation that both halves of this
session's work agree with each other and with B94's independently-derived scope. Report banked as
`MFM_v1_1_Reconciliation.md`, registered alongside `MFM_FW_Reconciliation.md`/`MFM_Standalone_Pass.md`.
Output is B89's work order.

**B95 opened, incidental.** While scoping the reconciliation report to "every built faction," the
operational definition had to be "has real rows in `units.json`," not `faction_taxonomy.json`'s `built`
flag — that flag marks Chaos Space Marines and Thousand Sons `built: false` even though `units.json`
already carries real roster data for both (Chaos Space Marines includes the very Rubric Marines
instance B87 corrected). Not investigated further or acted on this session — pure tooling/analysis
turn, and which is stale (the flag or the app's readiness) is a product call. Filed for a decision.
...
### D304 — Emperor's Children units shipped (S210), data-only

`EC` registered in `units_repro_check.py` (mirrors the Grey Knights/Thousand Sons block exactly) and
in `merge_factions.py`'s `--in` list. The real pipeline was run end to end, not a dry run:
`wahapedia_transform.py` (`--faction EC`) → `mfm_points_parser.py` (`MFM_Emperors_Children_v1.1.txt`,
per D293) → `convert_to_json.py --emit-fourth-plus` → `merge_factions.py` → `add_loadout_groups.py` →
`add_co_leader.py` → `add_bodyguard_stat_flags.py` → `add_chapter_point_overrides.py`. Diff-guarded
at unit-id level against the previously committed `units.json`: 23 EC units added, 0 removed, 0
changed elsewhere; `units_repro_check.py` passes byte-identical against the promoted file. Merged
lookups diff-guarded the same way: `abilities.json` +26 (all EC-only), `weapon_abilities.json` +1
(Icon of Excess), `rules.json`/`keywords.json` unchanged.

Confirmed by direct demonstration — not by trusting S209's scope doc — that the real build needed
far less manual authoring than estimated. Of the "6 manual" wargear groups and 5 "ambiguous"
weapon-name matches the transform-stage validation flagged, the real `loadout_parser.py` run (a
different, less conservative check than the transform's own validation report) resolved every one of
them cleanly: bare weapon names for all 5 ambiguous matches (matching how every other faction already
handles the identical plasma-pistol/plasma-gun/heavy-missile-launcher ambiguity — this is an
established project-wide convention, not a per-unit judgment call), `classify_n_model_swap`'s existing
compound-swap handling for Lord Kakophonist and both Noise Marines options, existing classifiers for
Maulerfiend and Chaos Rhino, and — checked directly against the raw option text — the Chaos Terminators
bundled swap needed no new `bundled_swaps.json` entry at all, since the plain "combi-bolter and
accursed weapon replaced with paired accursed weapons" compound already resolves via the existing
`replaces: "X + Y"` shape. EC's own Defiler (`000004208`, confirmed a separate datasheet ID from
CSM's `000000969`, DG's `000004209`, TS's `000001030`, and WE's `000004207` — each Chaos Legion carries
its own Defiler datasheet despite identical rules text) also parsed cleanly, matching its four already-
shipped siblings' structure exactly.

Only Tormentors (`000004079`) and Infractors (`000004080`) needed genuine hand-authoring. Both carry
the sentence "1 `<UnitName>` can be equipped with 1 icon of excess." — the generic word "model"
replaced with the unit's own singular name, a shape none of `loadout_parser.py`'s 19 `CLASSIFIERS`
match (`classify_one_model_add` hard-codes the literal word "model"). Checked the actual source text
before copying the Icon of Despair precedent's full structure (`requires_weapon`/`blocks_swap`): Icon
of Despair's real MFM text carries an explicit gating clause ("One Plague Marine equipped with a
boltgun can be equipped with 1 icon of despair. This model's boltgun cannot be replaced.") that Icon
of Excess's text does not have, so the two new entries use the plain `add` + `equipment` +
`max_total: 1` shape only. Both added to `repro_check.py`'s `HAND_AUTHORED` list; `EC` added to
`FACTIONS`; confirmed by direct demonstration (the real `--datasheets`-only `equipped_parser.py` pass,
the same one `repro_check.py` runs last) that no `Emperors_Children_web.txt` composition file is
needed, mirroring the Grey Knights precedent exactly — `WEB_PASSES` unchanged. `unit_loadouts.json`
regenerated via the real `loadout_parser.py` + `equipped_parser.py` chain: 21 auto-parsed EC units + 2
hand-authored added, 0 removed, 0 changed elsewhere; `repro_check.py` passes byte-identical. The
equipped-parser pass correctly enriched the two hand-authored entries' per-model-group
`default_weapons` split and added `_defaults_source: "equipped"` — the pipeline working as designed,
not a corruption of the hand-authored seed.

New structural assertion `EC-DATA` added, re-derived from source every run per the `B101-DATA`/
`B106-DATA` pattern: scans `Datasheets_options.csv` for the "N `<UnitName>` can be equipped with N
`<item>`" trigger and confirms every matching Emperor's Children datasheet carries the corresponding
add/equipment/max_total:1 option. Deliberately scoped to Emperor's Children only, not every
currently-built faction: the same sentence shape already exists, unfixed, on several other factions'
datasheets (Infiltrator Squad's helix gauntlet, Rubric Marines' icon of flame ×2, Incursor Squad's
haywire mine) as pre-existing `UNMATCHED` residuals — confirmed these are already-known backlog debt,
not something this session created or is responsible for fixing; scoping the assertion wide would have
failed the gate on gaps outside this session's turn. 122 assertions now registered (was 121).

`datasheet_wargear_abilities.json` regenerated — a real, previously-unflagged gap: 5 EC units
(`000004079`, `000004080`, `000004095`, `000004097`, `000004098`) were missing from the file entirely.
Diff-guarded: 5 added, 0 removed, 0 changed elsewhere.

`B61-2`/`B61-3` extended for Emperor's Children's Legions of Excess carriers. Confirmed directly
against `units.json`: 5 units (Shalaxi Helbane, Daemonettes, Fiends, Keeper of Secrets, Seekers) carry
`allied_group: "Legions of Excess"`, each with a distinct `local:chaos-daemons:*` id on the Chaos
Daemons side (not a merge collision) — the identical shape Death Guard's Plague Legions and Thousand
Sons' Scintillating Legions already use. `ALLIED_CARRIER_GROUPS` extended with the one-line entry its
own comment anticipated; both assertions' docstrings updated to name the third carrier army.

`E14-2`'s hardcoded count updated 90/61 → 98/67. Verified by faction breakdown before updating the
literal, not assumed: every non-EC faction's count is unchanged; Emperor's Children contributes
exactly 8 qualifying free-add options across 6 units — Tormentors' and Infractors' Icon of Excess (the
hand-authored EC-DATA shape), Chaos Land Raider's and Chaos Rhino's Havoc launcher, and Daemonettes'
and Seekers' Instrument of Chaos + Daemonic Icon pair (2 each).

**Found and opened B111, not fixed this session (would mix engine work into a data-only turn).**
Sourcing EC's Defiler wargear from `MFM_Emperors_Children_v1.1.txt` (per the scope doc's expectation of
a 15-pt price) returned zero items. Root cause, confirmed directly: every v1.1 MFM file dropped the
leading bullet character (`•`) from `WARGEAR OPTIONS` lines that v1_0 files have — checked
`MFM_Grey_Knights_v1.1.txt`, `MFM_Death_Guard_v1.1.txt`, `MFM_Thousand_Sons_v1.1.txt`, and
`MFM_Chaos_Space_Marines_v1.1.txt` directly; all show the same bullet-less format. `mfm_points_parser
.py`'s `WARGEAR_RE` regex hard-requires that bullet, so the `--wargear` pass has been silently blind to
v1.1 pricing for every faction since the v1.1 migration — not an EC-specific bug, and harmless
everywhere else purely because those factions' v1_0/v1.1 wargear item prices happen to be identical.
EC's Defiler is the first case where the two versions actually diverge (10 vs 15 pts), which is what
surfaced the gap. Shipped EC's Defiler wargear sourced from v1_0 (10 pts per item), consistent with its
already-shipped siblings (DG/TS/CSM Defilers, all currently also stuck at v1_0 pricing for the same
pre-existing reason) rather than leaving it unpriced (worse: a reachable item silently costing 0) or
hand-patching a number outside the pipeline. Diff-guarded: 1 unit added (`000004208`), 2 items, 0
removed, 0 changed elsewhere. `B111` tracks the regex fix as a dedicated tooling/engine turn.

**B110 corrected, not executed.** S209's B110 recommended flipping Grey Knights' `faction_taxonomy
.json` flag to `built: true`, reasoning that `units.json` shows 25/25 units complete. Checked
`detachments.json` directly this session (not assumed) and found Grey Knights has zero detachment
entries there. `built: true` is exactly the flag `index.html` uses to gate faction selectability
(`opt.disabled = !f.built`) — flipping it now would let a player select Grey Knights and reach an
empty detachment picker, a broken list-building state. Not executed; flagged for Ryan rather than
silently patched over, since the correct sequencing (Grey Knights detachments vs. Emperor's Children
detachments first) is a product-sequencing call, not a data question this session can resolve alone.
Grey Knights' taxonomy flag stays `false`.

Detachments and `faction_taxonomy.json` deliberately untouched this session — turn typing (this is a
